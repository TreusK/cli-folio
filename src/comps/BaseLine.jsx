...
function MiniLink({ str }) {
  if (str.includes('[[')) {
    let urlRegex = /\[\[(.*?)\]\]/g;
    let nameRegex = /\(\((.*?)\)\)/g;
    let urlArr = str.match(urlRegex);
    let nameArr = str.match(nameRegex);

    urlArr = urlArr.map(elem => elem.slice(2, elem.length-2));
    nameArr = nameArr.map(elem => elem.slice(2, elem.length-2));

    let cutIndex = str.indexOf('[');
    let copy = str.slice(0, cutIndex);
    return <p className='breakText'>
      {copy}
          {
            urlArr.map((elem, index) => <a key={elem} className='blockLink' href={elem} target="_blank">{nameArr[index]}</a>)
          }
      </p>
    //<p className='breakText'>{copy}<a href={url} target="_blank">{name}</a></p>
  }
  return (
